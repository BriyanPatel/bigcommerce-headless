...
const SubmitButton = () => {
  ...
  return (
    <Button
      className="relative w-fit items-center px-8 py-2 button primary-btn"
      data-button
      loading={pending}
...
export const ResetPasswordForm = ({ reCaptchaSettings }: Props) => {
  ...
  return (
    <>
      ...
      <p className="pass-intro">{t('description')}</p>

      <Form action={onSubmit} className="forgot-password-form" ref={form}>
          <FieldLabel className='form-label' htmlFor="email">{t('emailLabel')}</FieldLabel>
          <Field className="relative" name="email">
            ...
            <FieldMessage
              className="absolute error-message inset-x-0 bottom-0 inline-flex w-full text-sm text-gray-500"
              match="valueMissing"
            >
        ...
        {reCaptchaSettings?.isEnabledOnStorefront && (
          <Field className="relative g-captcha col-span-full max-w-full space-y-2 pb-7" name="ReCAPTCHA">
            <ReCaptcha
