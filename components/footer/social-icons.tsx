...
const SocialIcon = ({ name }: { name: string }) => {
  switch (name) {
    case 'Facebook':
      return <SiFacebook title="Facebook" style={{backgroundImage:`url("/components/footer/footer-assets/facebook.png")`}} />;
    ...
    <nav aria-label="Social media links" className="block">
      <ul className='socialLinks'>
        {socialMediaLinks.map((link) => {
          ...
          return (
            <li key={link.name}>
              <Link className={`inline-block social-icon ${link.name.toLowerCase()}-icon`} href={link.url}>
              {link.name}
