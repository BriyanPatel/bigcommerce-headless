...
const inputVariants = cva(
  'peer focus-visible:ring-primary/20 w-full py-2.5 px-4 text-base focus-visible:border-primary focus-visible:outline-none focus-visible:ring-4 hover:border-primary disabled:bg-gray-100 disabled:hover:border-gray-200',
  {
    variants: {
      variant: {
        success:
  ...
  ({ className, children, ...props }, ref) => {
    ...
    return (
      <span
        aria-hidden="true"
        className={cn(
          'error-icon pointer-events-none absolute end-4 top-0 flex h-full items-center peer-disabled:text-gray-200',
...
const Input = forwardRef<ElementRef<'input'>, InputProps>(
  ({ className, variant, children, type = 'text', ...props }, ref) => (
    <InputContext.Provider value={{ variant }}>
      <div className={cn('relative frm-input')}>
        <input
