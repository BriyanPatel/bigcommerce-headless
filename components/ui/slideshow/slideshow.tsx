      ...
      <SlideshowContext.Provider value={[emblaRef, emblaApi]}>
        <AutoplayContext.Provider value={[isPaused, togglePaused]}>
          <ActiveSlideContext.Provider value={[activeSlide, setActiveSlide]}>
            <div className="SliderMn">
              <section
                aria-label="Interactive slide show"
                aria-roledescription="carousel"
                className={cn(
                  'relative HeroCarousal-sec overflow-hidden lg:mx-0',
                  className,
                )}
...
const SlideshowControls = forwardRef<ElementRef<'div'>, ComponentPropsWithRef<'div'>>(
  ({ children, className, ...props }, ref) => {
    ...
    return (
      <div
        className={cn('arrows-BTN', className)}
        ref={ref}
        {...props}
      >
        {children}
  ...
  ({ children, className, onClick, ...props }, ref) => {
    ...
    return (
      <button
        ...
        className={cn(
          'inline-flex h-12 arrows Next-slide w-12 items-center justify-center focus-visible:outline-none focus-visible:ring-4 focus-visible:ring-primary/20',
          className,
        )}
        onClick={scrollNext}
        ref={ref}
...
>(({ children, className, onClick, ...props }, ref) => {
  ...
  return (
    <button
      ...
      className={cn(
        'inline-flex h-12 arrows Previous-slide w-12 items-center justify-center focus-visible:outline-none focus-visible:ring-4 focus-visible:ring-primary/20',
        className,
      )}
      onClick={scrollPrev}
      ref={ref}
