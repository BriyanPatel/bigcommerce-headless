  ...
  return (
    <ExpandedContext.Provider value={{ isExpanded, setIsExpanded: toggleExpanded }}>
      <NavigationMenuPrimitive.Root
        className={cn(isExpanded && 'h-screen overflow-y-auto mm-menu-lst')}
      ...
      >
        <FocusTrap active={isExpanded}>
          <div className="relative">
            <div className={cn( 'header_wrapper group flex  items-center justify-between gap-1 overflow-y-visible 2xl:container sm:px-10 lg:gap-8 lg:px-12 2xl:mx-auto 2xl:px-0',
                className, )}
            >   
              {children}
            </div>
            {!isExpanded && (
              <NavigationMenuPrimitive.Viewport
                className={cn(
                  'mega-menu absolute start-0 top-full z-50 w-full shadow-xl duration-200 animate-in slide-in-from-top-5',
                )}
              />
            )}
          </div>
  ...
  ({ children, className, onClick, ...props }, ref) => {
    ...
    return (
      <button
        ...
        className={cn( 'mm-menu group p-3 hover:text-primary focus-visible:outline-none focus-visible:ring-4 focus-visible:ring-primary/20',
          className,
        )}
        onClick={(e) => {
          onClick?.(e);
...
const NavigationMenuCollapsed = forwardRef<ElementRef<'div'>, ComponentPropsWithRef<'div'>>(
  ({ children, className, ...props }, ref) => {
    ...
    return (
      <div
        className={cn(
          'in-collapsed-nav group absolute start-0 top-full z-50 w-full px-4 pb-6 duration-200 animate-in slide-in-from-top-5 2xl:container sm:px-10 lg:px-12 2xl:mx-auto 2xl:px-0',
          className,
          !isExpanded && 'hidden',
        )}
        id="nav-menu"
